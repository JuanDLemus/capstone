...
export default function Reg3Screen({ navigation, route }) {
  ...
  const handleContinue = () => {
    navigation.navigate('Reg4', {
      ...prevData,
      accessibility: {
        preferred_input_mode: toggles.mic ? 'voice' : (toggles.aac ? 'aac' : 'chat'),
        high_contrast: toggles.hc,
        reduce_motion: false,
        screen_reader_optimized: toggles.lt,
      },
    });
  };
...
const styles = StyleSheet.create({
  ...
  scroll: { flex: 1 },
  card: { backgroundColor: T.surf, borderRadius: 24, paddingVertical: 12, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8 },
  row: { flexDirection: 'row', alignItems: 'center', paddingVertical: 16, paddingHorizontal: 20 },
  rowBorder: { borderBottomWidth: 1, borderBottomColor: T.n100 },
  rowInfo: { flex: 1 },
  rowLabel: { fontFamily: T.fontNunito, fontSize: 16, fontWeight: T.w8, color: T.n900 },
  rowSub: { fontFamily: T.fontNunito, fontSize: 12, color: T.n500, marginTop: 2 },
